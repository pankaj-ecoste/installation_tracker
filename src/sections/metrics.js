...
export function renderMetrics(){
  ...
  const onHold=Object.values(projectGroups).filter(towers=>!towers.every(p=>p.status==='Completed')&&!towers.some(p=>p.status==='In Progress'||p.status==='Completed')&&towers.some(p=>p.status==='On Hold')).length;
  ...
  document.getElementById('metrics-row').innerHTML=
    ...
    '<div class="metric"><div class="metric-label">In progress</div><div class="metric-val">'+ip+'</div><div class="metric-sub">'+(total-done-ip-onHold)+' not started</div></div>'+
